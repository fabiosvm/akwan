...
static void do_global(AkwVM *vm, AkwChunk *chunk, uint8_t *ip, AkwValue *slots);
...
static void do_call(AkwVM *vm, AkwChunk *chunk, uint8_t *ip, AkwValue *slots);
...
static AkwInstructionHandleFn instructionHandles[] = {
  ...
  [AKW_OP_POP]       = do_pop,       [AKW_OP_GLOBAL]      = do_global,      [AKW_OP_GET_LOCAL] = do_get_local,
